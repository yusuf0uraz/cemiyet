...
interface EventsState {
  ...
  fetchEvents: (params?: { cat?: string; q?: string }) => Promise<void>;
...
export const useEventsStore = create<EventsState>((set, get) => ({
  ...
  fetchEvents: async (params) => {
    ...
    try {
      ...
      if (__DEV__) console.warn('[Events] fetchEvents hata:', err?.message);
      set({ loading: false });
    }
  },
  ...
  fetchBookmarks: async () => {
    try {
      ...
      if (__DEV__) console.warn('[Events] fetchBookmarks hata:', err?.message);
    }
  },

  fetchJoined: async () => {
    try {
      const raw = await eventsService.getMyJoined();
      const ids = (raw as unknown as Record<string, unknown>[]).map(e => e.id as string);
      set({ joinedEvents: ids });
    } catch (err: any) {
      if (__DEV__) console.warn('[Events] fetchJoined hata:', err?.message);
    }
  },
  ...
  createEvent: async (payload) => {
    ...
    try {
      ...
      if (__DEV__) console.warn('[Events] createEvent hata:', err?.message);
      get().addEvent({
