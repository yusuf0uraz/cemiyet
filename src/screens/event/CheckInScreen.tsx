...
type Moment = { user: string; tone: '1'|'2'|'3'|'4'|'5'; text: string; time: string };

const DEMO_MOMENTS: Moment[] = [
  { user: 'Ahmet K.', tone: '1', text: 'Harika bir başlangıç! 🎾', time: '14:32' },
  { user: 'Selin Y.', tone: '3', text: 'Set sayısı 2-1 oldu.', time: '14:45' },
  { user: 'Burak T.', tone: '2', text: 'Final başlıyor!', time: '15:10' },
];
...
export function CheckInScreen({ navigation, route }: Props) {
  ...
  const handleCheckIn = async () => {
    ...
    setUploading(true);
    try {
      const url = await uploadImage(uri, 'event');
      setMyPhotos(prev => [url, ...prev]);
    } catch {
  ...
  return (
    ...
    >
      <SafeAreaView edges={['top']} style={{ backgroundColor: 'transparent' }}>
        <Animated.View entering={FadeInDown.duration(300)} style={styles.header}>
          ...
          <View style={styles.titleArea}>
            <LiveBadge />
            <Text style={styles.eventTitle} numberOfLines={1}>{event?.title ?? 'Canlı Etkinlik'}</Text>
          </View>
      ...
      >
        {/* Ana fotoğraf */}
        <Animated.View entering={FadeInDown.delay(60).duration(400)} style={styles.mainPhotoWrap}>
          <PhotoSlot tone="1" height={280} width="100%">
            <LinearGradient colors={['transparent', 'rgba(14,11,8,0.9)']} style={StyleSheet.absoluteFill} />
            ...
            <View style={styles.mainPhotoBottom}>
              <View style={styles.reactionRow}>
                <TouchableOpacity style={[styles.reactBtn, liked && styles.reactBtnActive]} onPress={() => setLiked(v => !v)}>
                  {liked ? <IcnHeartFill size={18} color={colors.ember} /> : <IcnHeart size={18} color="#fff" />}
                  <Text style={styles.reactCount}>47</Text>
                </TouchableOpacity>
                <TouchableOpacity style={[styles.reactBtn, flamed && styles.reactBtnActive]} onPress={() => setFlamed(v => !v)}>
                  {flamed ? <IcnFlameFill size={18} color={colors.amber} /> : <IcnFlame size={18} color="#fff" />}
        ...
        <Animated.View entering={FadeInDown.delay(120).duration(400)}>
          <View style={styles.photoStreamHeader}>
            ...
            <TouchableOpacity style={styles.addPhotoBtn} onPress={handleCheckIn} disabled={uploading}>
              <IcnCamera size={16} color={colors.ember} />
              <Text style={styles.addPhotoText}>
                {uploading ? 'Yükleniyor...' : checkedIn ? '✓ Check-in' : 'Check-in & Fotoğraf'}
              </Text>
            </TouchableOpacity>
          </View>
          <ScrollView
          ...
          >
            {/* Kendi fotoğraflarım */}
            {myPhotos.map((uri, i) => (
              <Animated.View key={`my-${i}`} entering={SlideInRight.delay(i * 80)}>
                <TouchableOpacity style={styles.streamPhotoWrap} activeOpacity={0.9}>
            ...
            {/* Demo fotoğraflar */}
            {DEMO_PHOTOS.map(({ tone, user: photoUser, time }, i) => (
              <Animated.View key={i} entering={SlideInRight.delay((myPhotos.length + i) * 60)}>
                <TouchableOpacity style={styles.streamPhotoWrap} activeOpacity={0.9}>
            ...
            <TouchableOpacity style={styles.addPhotoCard} activeOpacity={0.85} onPress={handleCheckIn}>
              <LinearGradient colors={[colors.ember, '#C03A1F']} style={[StyleSheet.absoluteFill, { borderRadius: r.md }]} />
      ...
      {/* Alt input — klavye ile birlikte yukarı kayar */}
      <Animated.View entering={FadeInUp.delay(300)} style={styles.bottomInput}>
...
const styles = StyleSheet.create({
  ...
  eventTitle: { fontFamily: 'Manrope_700Bold', fontSize: fontSizes.lg, color: '#fff' },
  attendCount: {
    flexDirection: 'row', alignItems: 'center', gap: 4,
    backgroundColor: 'rgba(255,255,255,0.12)', paddingHorizontal: 10, paddingVertical: 6, borderRadius: r.pill,
  },
  attendText: { fontFamily: 'Manrope_700Bold', fontSize: fontSizes.md, color: 'rgba(255,255,255,0.8)' },
  ...
  durationText: { fontFamily: 'JetBrainsMono_500Medium', fontSize: fontSizes.sm, color: '#fff' },
  mainPhotoBottom: { position: 'absolute', bottom: 14, left: 14 },
  ...
  reactCount: { fontFamily: 'Manrope_700Bold', fontSize: fontSizes.md, color: '#fff' },
  photoStreamHeader: {
    flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center',
    paddingHorizontal: 16, paddingTop: 20, paddingBottom: 12,
  },
  photoStreamTitle: { fontFamily: 'Manrope_800ExtraBold', fontSize: fontSizes['5xl'], color: '#fff' },
  addPhotoBtn: {
    flexDirection: 'row', alignItems: 'center', gap: 6,
    backgroundColor: 'rgba(232,76,44,0.2)', paddingHorizontal: 12, paddingVertical: 7, borderRadius: r.pill,
  },
  addPhotoText: { fontFamily: 'Manrope_700Bold', fontSize: fontSizes.md, color: colors.ember },
  photoStream: { paddingHorizontal: 16, gap: 10 },
  streamPhotoWrap: {},
  streamPhotoMeta: { position: 'absolute', bottom: 8, left: 8 },
  streamPhotoUser: { fontFamily: 'Manrope_700Bold', fontSize: fontSizes.sm, color: '#fff' },
  streamPhotoTime: { fontFamily: 'Manrope_500Medium', fontSize: fontSizes.xs, color: 'rgba(255,255,255,0.7)' },
  addPhotoCard: {
    width: 105, height: 140, borderRadius: r.md,
    alignItems: 'center', justifyContent: 'center', gap: 8, overflow: 'hidden',
  },
  addPhotoCardText: { fontFamily: 'Manrope_700Bold', fontSize: fontSizes.md, color: '#fff', textAlign: 'center' },
  momentsHeader: {
    flexDirection: 'row', alignItems: 'center', gap: 8,
    paddingHorizontal: 16, paddingTop: 20, paddingBottom: 12,
  },
  momentsTitle: { fontFamily: 'JetBrainsMono_500Medium', fontSize: fontSizes.xs, color: colors.amber, letterSpacing: 0.5 },
  momentsList: { paddingHorizontal: 16, gap: 12 },
  momentRow: { flexDirection: 'row', gap: 10, alignItems: 'flex-start' },
  momentBubble: { flex: 1, backgroundColor: 'rgba(255,255,255,0.08)', borderRadius: r.md, padding: 12 },
  momentMeta: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: 4 },
  momentUser: { fontFamily: 'Manrope_700Bold', fontSize: fontSizes.md, color: '#fff' },
  momentTime: { fontFamily: 'JetBrainsMono_500Medium', fontSize: fontSizes.xs, color: 'rgba(255,255,255,0.4)' },
  momentText: { fontFamily: 'Manrope_500Medium', fontSize: fontSizes.lg, color: 'rgba(255,255,255,0.9)' },
  bottomInput: {
    borderTopWidth: 1, borderTopColor: 'rgba(255,255,255,0.1)',
    padding: 12, paddingBottom: Platform.OS === 'ios' ? 28 : 14,
    backgroundColor: 'rgba(14,11,8,0.97)',
  },
  inputRow: { flexDirection: 'row', alignItems: 'center', gap: 10 },
  inputBox: {
    flex: 1, backgroundColor: 'rgba(255,255,255,0.12)', borderRadius: r.pill,
    paddingHorizontal: 16, paddingVertical: 10,
    fontFamily: 'Manrope_500Medium', fontSize: fontSizes.lg,
    maxHeight: 100, color: '#fff' as string,
  },
  sendBtn: {
